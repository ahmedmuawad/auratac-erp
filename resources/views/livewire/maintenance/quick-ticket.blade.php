<div class="max-w-5xl mx-auto space-y-6 pb-16">
    ...
    <div class="bg-onyx rounded-md-xl p-8 flex flex-col md:flex-row justify-between items-center gap-5">
        <div class="text-center md:text-start">
            <h1 class="text-headline text-on-onyx">الاستقبال السريع</h1>
            <p class="text-body text-on-onyx-variant mt-1">سجّل البيانات واطبع كرت الاستلام في ثوانٍ</p>
        </div>
        <div class="flex items-center gap-3 bg-white/5 px-4 py-3 rounded-md-md">
            <div class="text-end">
                <p class="text-label-sm text-on-onyx-variant uppercase tracking-widest">تاريخ اليوم</p>
    ...
    <form wire:submit="save" class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="md-card-elevated p-6 space-y-5">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                        <label class="md-label">رقم الجوال (بحث ذكي)</label>
                    ...
                    <div>
                        <label class="md-label">اسم العميل بالكامل</label>
                    ...
                    <div class="md:col-span-2">
                        <label class="md-label">رقم الهوية / الإقامة (اختياري)</label>
            ...
            <div class="md-card-elevated p-6 space-y-5">
                <h3 class="text-title-lg text-on-surface flex items-center gap-3 border-b pb-4" style="border-color:var(--md-outline-variant)">
                    ...
                    بيانات السلاح / القطعة
                </h3>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                        <label class="md-label">نوع السلاح</label>
                        <input wire:model="item_name" type="text" class="md-field rounded-md-sm" placeholder="مثلاً: مسدس">
                        @error('item_name') <span class="text-label-sm text-error">{{ $message }}</span> @enderror
                    </div>
                    <div>
                        <label class="md-label">الرقم التسلسلي (Serial)</label>
                        <input wire:model="item_serial" type="text" class="md-field rounded-md-sm">
                    </div>
                    <div>
                        <label class="md-label">الماركة / المصنع</label>
                        <input wire:model="item_brand" type="text" class="md-field rounded-md-sm">
                    </div>
                    <div>
                        <label class="md-label">رقم الرخصة (إن وجد)</label>
        ...
        <div class="space-y-6">
            ...
            <div class="md-card-elevated p-6 text-center space-y-4">
                <h4 class="text-label text-on-surface-variant uppercase tracking-widest">توثيق الحالة (صورة)</h4>
                <div class="relative group mx-auto w-44 h-44 rounded-md-lg bg-surface-container border-2 border-dashed flex items-center justify-center overflow-hidden" style="border-color:var(--md-outline-variant)">
                    ...
                    @else
                        <label for="take-photo" class="cursor-pointer flex flex-col items-center gap-2 text-on-surface-variant">
                            ...
                            <span class="text-label-sm">اضغط للتصوير</span>
                        </label>
                        <input type="file" wire:model="item_photo" class="hidden" id="take-photo" accept="image/*" capture="environment">
                    @endif
                </div>
                <div wire:loading wire:target="item_photo" class="text-label-sm text-primary">جاري المعالجة...</div>
                @error('item_photo') <span class="text-label-sm text-error">{{ $message }}</span> @enderror
            </div>

            {{-- Financials --}}
            <div class="md-card-elevated p-6 space-y-5">
                <h4 class="text-label text-on-surface-variant uppercase tracking-widest border-b pb-3" style="border-color:var(--md-outline-variant)">التقدير المالي والدفع</h4>

                <div class="p-4 bg-onyx rounded-md-md flex justify-between items-center">
                    <span class="text-label text-on-onyx-variant">إجمالي التقدير</span>
                    <span class="text-title-lg text-primary tabular-nums">{{ number_format((float)($expected_cost_labor ?? 0) + (float)($expected_cost_parts ?? 0), 2) }} <small class="text-label">SAR</small></span>
                </div>

                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="md-label">أجور اليد</label>
                        <input wire:model.live="expected_cost_labor" type="number" class="md-field rounded-md-sm" placeholder="0.00">
                    </div>
                    <div>
                        <label class="md-label">تكلفة القطع</label>
                        <input wire:model.live="expected_cost_parts" type="number" class="md-field rounded-md-sm" placeholder="0.00">
                    </div>
                </div>

                <div>
                    <label class="md-label text-primary">المبلغ المدفوع (تحت الحساب)</label>
                ...
                @if($rem > 0)
                    <div class="md-status bg-warning-container text-on-warning-container w-full justify-between">
                        ...
                        <span class="tabular-nums">{{ number_format($rem, 2) }} SAR</span>
                    </div>
                @elseif($total > 0)
            ...
            <button type="submit" wire:loading.attr="disabled" class="md-btn md-btn-filled w-full h-14 text-title">
                <span wire:loading.remove>حفظ وطباعة الكرت</span>
